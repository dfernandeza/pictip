...
"")),spot=null,$spot=null,tooltip=null,$tooltip;for(i=0;i<opts.spots.length;i++){spot=new Spot(i,opts.spots[i]);tooltip=new ToolTip(i,opts.spots[i],opts.tooltip);spot.tooltip=tooltip;$spot=spot.create(i,spotTemplate);spots.push(spot);$spots.push($spot[0]);$tooltip=tooltip.create();$tooltips.push($tooltip[0])}$el.append($spots.concat($tooltips));$el.find(opts.spotClass).fadeIn(200);attachEventHandlers.call(this)};$el.data("pictip",this)};$.fn.pictip=function(options){return this.each(function(){(new PicTip(this)).init(options)})};
$.fn.pictip.options={spots:[],spotClass:".spot",spotTemplate:"<a href='#'></a>",eventType:"click",tooltip:true,onShowToolTip:function(spot,tooltip){},onCloseToolTip:function(spot,tooltip){}}})(window.jQuery||window.Zepto);